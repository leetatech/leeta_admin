...
const DeclineRequest: React.FC<DeclineRequestProps> = ({ isOpen, onClose, onSubmit }) => {
  ...
  return (
    <>
        ...
        <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50'>
          <div className='bg-white font-title rounded-t-lg p-6 w-96 flex justify-center flex-col'>
            ...
            <div className='flex flex-col gap-3'>
              ...
              <button onClick={() => setShowWhatWentWrong(true)} className='bg-[#DC0121] text-[15px] font-normal text-white rounded-lg py-2 hover:bg-[#b30018] transition-colors'>
                Decline Order
              </button>
            </div>
          </div>
      ...
      {showWhatWentWrong && <WhatWentWrong closeDecline={onClose} onClose={() => setShowWhatWentWrong(false)} onSubmit={onSubmit} />}
    </>
  );
};
