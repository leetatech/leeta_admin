...
import { resetStatusUpdate, setAction, setDetailsGenState, triggerOrderList, triggerOrderUpdate } from '../../features/orders/order_slice';
                        ...
                        <tr className={`${expandedRow === row.id ? 'bg-[#EAECF0]' : 'hover:bg-[#EAECF0] transition'}`}>
                          ...
                          {!expandedRow && (
                            <>
                              ...
                              <td className='border-b border-[#EAECF0] p-2'>
                                <button className='flex gap-4 items-center text-[#141388]' onClick={() => handleDetailsClick(Number(row.order_number))}>
            ...
            {expandedRow && (
              <div className='w-[70%] h-[70vh] bg-gray-100 p-6 border border-[#EAECF0] transition-transform duration-300 ease-in-out ' style={{ overflowY: 'auto' }}>
                ...
                <div className='flex gap-2 items-center mt-2'>
                  ...
                  <Typography variant={TypographyVariant.BODY_SMALL_MEDIUM}>{details?.delivery_details?.name}</Typography>
                </div>
                <div className='flex gap-2 items-center mt-2'>
                  <img src={phone} alt='phone' className='h-4 w-4' />
                  <Typography variant={TypographyVariant.BODY_SMALL_MEDIUM}>{details?.delivery_details?.phone}</Typography>
                </div>
                <div className='flex gap-2 items-center mt-2'>
                ...
                <div className='p-2'>
                  ...
                  <div className='flex p-1 justify-between'>
                    ...
                    <Typography variant={TypographyVariant.BODY_SMALL_MEDIUM}>{Number(details?.orders?.reduce((accum: any, item: any) => accum + item.cost, 0)).toLocaleString()}</Typography>
                  </div>
                  <div className='flex p-1 justify-between'>
                    <Typography variant={TypographyVariant.BODY_SMALL_MEDIUM}>Delivery Fee</Typography>
                    <Typography variant={TypographyVariant.BODY_SMALL_MEDIUM}>{Number(details?.delivery_fee).toLocaleString()}</Typography>
                  </div>
                  <div className='flex p-1 justify-between'>
                    <Typography variant={TypographyVariant.BODY_SMALL_MEDIUM}>Service Fee</Typography>
                    <Typography variant={TypographyVariant.BODY_SMALL_MEDIUM}>{Number(details?.service_fee).toLocaleString()}</Typography>
                  </div>
                  <div className='flex p-1 justify-between'>
                    <p className='font-bold text-14px'>Total</p>
                    <p className='text-[#3EAF3F] leading-[26px] font-medium'>{Number(details?.total).toLocaleString()}</p>
                  </div>
                </div>
                {activeTab === 'PENDING' && (
                  <div className='flex justify-center gap-40 mt-2 mb-2'>
                    <button className='border border-[#EF0F30] bg-red-100 text-[#EF0F30] px-14 py-2' onClick={() => setIsModalOpen(true)}>
